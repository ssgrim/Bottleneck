...
function Test-BottleneckEventLog {
    try {
        ...
        foreach ($logName in $logs) {
            # Get critical errors from last 30 days with guards
            $analysisStart = (Get-Date).AddDays(-30)
            $errors = Invoke-WithTimeout -TimeoutSeconds 15 -ScriptBlock {
                param($ln, $st)
                if (-not $ln) { return @() }
                if (-not $st) { $st = (Get-Date).AddDays(-30) }
                Get-WinEvent -FilterHashtable @{ LogName=$ln; Level=1,2; StartTime=$st } -MaxEvents 500 -ErrorAction SilentlyContinue
            } -ArgumentList $logName, $analysisStart

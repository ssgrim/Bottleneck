...
function Test-BottleneckBootTime {
    try {
        ...
        $bootEvents = Invoke-WithTimeout -TimeoutSeconds 10 -ScriptBlock {
            $start = if ($using:lastBoot) { $using:lastBoot } else { (Get-Date).AddDays(-1) }
            Get-WinEvent -FilterHashtable @{
                LogName='System'; ProviderName='Microsoft-Windows-Diagnostics-Performance'; Id=100; StartTime=$start
        ...
        $slowBootEvents = Invoke-WithTimeout -TimeoutSeconds 10 -ScriptBlock {
            $start = if ($using:lastBoot) { $using:lastBoot } else { (Get-Date).AddDays(-1) }
            Get-WinEvent -FilterHashtable @{
                LogName='System'; ProviderName='Microsoft-Windows-Diagnostics-Performance'; Id=101,102,103; StartTime=$start
            } -MaxEvents 20 -ErrorAction SilentlyContinue
        }

        $slowComponents = @()
        foreach ($sbComp in $slowBootEvents) {
            if ($sbComp.Properties.Count -gt 0) {
                $slowComponents += $sbComp.Properties[0].Value
            }
        }
...
function Test-BottleneckAppLaunchPerformance {
    try {
        ...
        $appStart = (Get-Date).AddDays(-7)
        $slowAppEvents = Get-WinEvent -FilterHashtable @{
            LogName='Application'; Level=2,3; StartTime=$appStart
...
function Test-BottleneckUIResponsiveness {
    try {
        ...
        $hangStart = (Get-Date).AddDays(-7)
        $hangEvents = Invoke-WithTimeout -TimeoutSeconds 10 -ScriptBlock {
            Get-WinEvent -FilterHashtable @{ LogName='Application'; ProviderName='Application Hang'; StartTime=$using:hangStart } -MaxEvents 100 -ErrorAction SilentlyContinue
        }

        $hangCount = if ($hangEvents) { $hangEvents.Count } else { 0 }

        # Check for DWM (Desktop Window Manager) issues
        $dwmStart = (Get-Date).AddDays(-7)
        $dwmEvents = Invoke-WithTimeout -TimeoutSeconds 10 -ScriptBlock {
            Get-WinEvent -FilterHashtable @{ LogName='System'; ProviderName='Desktop Window Manager'; Level=2,3; StartTime=$using:dwmStart } -MaxEvents 100 -ErrorAction SilentlyContinue
        }

        $dwmIssues = if ($dwmEvents) { $dwmEvents.Count } else { 0 }

        # Check for system responsiveness events
        $lagStart = (Get-Date).AddDays(-7)
        $lagEvents = Invoke-WithTimeout -TimeoutSeconds 10 -ScriptBlock {
            Get-WinEvent -FilterHashtable @{ LogName='System'; Id=2004,2006; StartTime=$using:lagStart } -MaxEvents 50 -ErrorAction SilentlyContinue
        }

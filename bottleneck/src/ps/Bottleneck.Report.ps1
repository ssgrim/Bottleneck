...
function Invoke-BottleneckReport {
    ...
    $rca = $null
    try { $rca = Invoke-BottleneckNetworkRootCause -ErrorAction SilentlyContinue } catch {}
    $csvDiag = $null
    try { $csvDiag = Invoke-BottleneckNetworkCsvDiagnostics -ErrorAction SilentlyContinue } catch {}
    # Load latest path-quality (MTR-lite) summary if available
    $pathQuality = $null
    try {
        $pqFile = Get-ChildItem -Path $ReportsPath -Filter 'path-quality-*.json' | Sort-Object LastWriteTime -Descending | Select-Object -First 1
        if ($pqFile) { $pathQuality = (Get-Content $pqFile.FullName | ConvertFrom-Json) }
    } catch {}
    
    # Load speedtest history
    $speedHistory = @()
    try {
        $speedFile = Join-Path $ReportsPath 'speedtest-history.json'
        if (Test-Path $speedFile) { $speedHistory = (Get-Content $speedFile | ConvertFrom-Json) | Select-Object -Last 10 }
    } catch {}
    
    # Capture live traffic snapshot
    $trafficSnapshot = $null
    try {
        Write-Verbose "Capturing network traffic snapshot..."
        $trafficSnapshot = Get-BottleneckNetworkTrafficSnapshot -DurationSeconds 5
    } catch {}

    # Pre-compute RCA metrics for here-string
    $rcaSuccessRate = if($rca){$rca.Summary.SuccessPct}else{0}
    $rcaP95Latency = if($rca){$rca.Summary.P95LatencyMs}else{0}
    $rcaDrops = if($rca){$rca.Summary.Drops}else{0}
    $rcaCause = if($rca){$rca.LikelyCause}else{'N/A'}
...
<div class="metric-card"><div class="metric-label">Success Rate</div><div class="metric-value">$("{0:N2}%" -f $rcaSuccessRate)</div></div>
<div class="metric-card"><div class="metric-label">P95 Latency</div><div class="metric-value">$("{0:N1} ms" -f $rcaP95Latency)</div></div>
<div class="metric-card"><div class="metric-label">Drops</div><div class="metric-value">$rcaDrops</div></div>
<div class="metric-card"><div class="metric-label">Likely Cause</div><div class="metric-value">$rcaCause</div></div>
</div>
<h3>Recommendations</h3>
<ul>$(if($rca -and $rca.Recommendations){ ($rca.Recommendations | ForEach-Object { "<li>$_</li>" }) -join "" } else { '<li>No recommendations</li>' })</ul>
$(if($csvDiag){
"<h3>Per-Target Latency Analysis</h3>
<table style='width:100%; font-size:12px;'>
<tr><th>Target</th><th>Samples</th><th>Success %</th><th>Avg (ms)</th><th>P50</th><th>P95</th><th>P99</th><th>Max</th><th>Drops</th></tr>
$( ($csvDiag.PerTargetStats | Where-Object { $_.AvgLatencyMs -gt 0 } | ForEach-Object { "<tr><td>$($_.Target)</td><td>$($_.Samples)</td><td>$($_.SuccessPct)</td><td>$($_.AvgLatencyMs)</td><td>$($_.P50)</td><td>$($_.P95)</td><td>$($_.P99)</td><td>$($_.MaxLatencyMs)</td><td>$($_.Drops)</td></tr>" }) -join '' )
</table>
<h3>Host Comparison</h3>
<p><strong>Best:</strong> $($csvDiag.HostComparison.BestTarget) ($($csvDiag.HostComparison.BestAvgMs) ms avg) &nbsp;|&nbsp; <strong>Worst:</strong> $($csvDiag.HostComparison.WorstTarget) ($($csvDiag.HostComparison.WorstAvgMs) ms avg) &nbsp;|&nbsp; <strong>Differential:</strong> $($csvDiag.HostComparison.DifferentialMs) ms</p>
<h3>Top Jitter Minutes</h3>
<table style='width:100%; font-size:12px;'>
<tr><th>Minute</th><th>Samples</th><th>Avg (ms)</th><th>Jitter (ms)</th></tr>
$( ($csvDiag.TopJitterMinutes | ForEach-Object { "<tr><td>$($_.Minute)</td><td>$($_.Samples)</td><td>$($_.AvgMs)</td><td>$($_.JitterMs)</td></tr>" }) -join '' )
</table>
<h3>Latency Spike Summary</h3>
<table style='width:100%; font-size:12px;'>
<tr><th>Target</th><th>Spike Minutes</th><th>Worst Spike (ms)</th><th>Threshold (ms)</th></tr>
$( ($csvDiag.SpikeMinutes | Group-Object Target | ForEach-Object { $worst = ($_.Group | Sort-Object MaxSpike -Descending | Select-Object -First 1); "<tr><td>$($_.Name)</td><td>$($_.Count)</td><td>$($worst.MaxSpike)</td><td>$($worst.Threshold)</td></tr>" }) -join '' )
</table>"
}else{
"<h3>Multi-host Probes</h3>
<table><tr><th>Target</th><th>Avg Latency (ms)</th></tr>$(if(`$rca -and `$rca.Probes){ (`$rca.Probes | ForEach-Object { `"<tr><td>`$(`$_.Target)</td><td>`$([string]::Format('{0:N1}', `$_.AvgLatencyMs))</td></tr>`" }) -join `"`" } else { '<tr><td colspan=`"2`">No probe data</td></tr>' })</table>
<h3>Recent Jitter (last 10 minutes)</h3>
<table><tr><th>Minute</th><th>Samples</th><th>Avg (ms)</th><th>Jitter (ms)</th></tr>$(if(`$rca -and `$rca.JitterByMinute){ (`$rca.JitterByMinute | Sort-Object Minute | Select-Object -Last 10 | ForEach-Object { `"<tr><td>`$(`$_.Minute)</td><td>`$(`$_.Count)</td><td>`$(`$_.AvgMs)</td><td>`$(`$_.JitterMs)</td></tr>`" }) -join `"`" } else { '<tr><td colspan=`"4`">No jitter data</td></tr>' })</table>"
})
<h3>Failure Clusters (>=3/min)</h3>
<table><tr><th>Minute</th><th>Failures</th></tr>$(if($rca -and $rca.FailureClusters -and $rca.FailureClusters.Count -gt 0){ ($rca.FailureClusters | ForEach-Object { "<tr><td>$($_.Name)</td><td>$($_.Count)</td></tr>" }) -join "" } else { '<tr><td colspan="2">None detected</td></tr>' })</table>
<h3>Traceroute Snapshot</h3>
<table><tr><th>Hop</th><th>Address</th></tr>$(if($rca -and $rca.TraceRoute){ ($rca.TraceRoute | ForEach-Object { "<tr><td>$($_.Hop)</td><td>$($_.NodeName)</td></tr>" }) -join "" } else { '<tr><td colspan="2">No traceroute data</td></tr>' })</table>
</div>
<div class="meta">
...
"@
    ...
    # Append Path Quality section if available
    $pathQualitySection = ''
    if ($pathQuality -and $pathQuality.Hops) {
        $rows = ($pathQuality.Hops | Sort-Object @{Expression='LossPct';Descending=$true}, @{Expression='P95Ms';Descending=$true} | ForEach-Object {
            "<tr><td>$($_.HopIndices)</td><td>$($_.IP)</td><td>$($_.Samples)</td><td>$($_.AvgMs)</td><td>$($_.P95Ms)</td><td>$($_.LossPct)</td></tr>"
        }) -join ''
        $target = if ($pathQuality.Target) { $pathQuality.Target } else { 'N/A' }
        $runs = if ($pathQuality.Runs) { $pathQuality.Runs } else { 0 }
        $pathQualitySection = @"
<div class='section'>
<h2>Path Quality (MTR-lite)</h2>
<div class="metrics-grid">
  <div class="metric-card"><div class="metric-label">Primary Target</div><div class="metric-value">$target</div></div>
  <div class="metric-card"><div class="metric-label">Traceroute Runs</div><div class="metric-value">$runs</div></div>
</div>
<table style='width:100%; font-size:12px;'>
<tr><th>Hop</th><th>IP</th><th>Samples</th><th>Avg (ms)</th><th>P95 (ms)</th><th>Loss %</th></tr>
$rows
</table>
</div>
"@
        $html += $pathQualitySection
    }
    
    # Append Traffic Overview section if available
    if ($trafficSnapshot) {
        $procRows = if ($trafficSnapshot.TopProcesses) {
            ($trafficSnapshot.TopProcesses | ForEach-Object {
                $risk = if ($_.Ports) { $riskyFound = @(21,23,25,110,135,139,445,1433,3389,5900) | Where-Object { $_.ToString() -in ($_.Ports -split ', ') }; if ($riskyFound) { "<span class='badge badge-danger'>⚠ Risky</span>" } else { '' } } else { '' }
                "<tr><td>$($_.ProcessName)</td><td>$($_.ProcessId)</td><td>$($_.ConnectionCount)</td><td style='font-size:10px;'>$($_.RemoteAddresses)</td><td>$($_.Ports)</td><td>$risk</td></tr>"
            }) -join ''
        } else { "<tr><td colspan='6'>No active connections</td></tr>" }
        
        $riskyPortWarning = if ($trafficSnapshot.RiskyPortsDetected -and $trafficSnapshot.RiskyPortsDetected.Count -gt 0) {
            "<p class='badge badge-danger' style='display:block;margin-top:10px;'>⚠️ Risky ports detected: $($trafficSnapshot.RiskyPortsDetected -join ', ')</p>"
        } else { '' }
...
<div class="metrics-grid">
  <div class="metric-card"><div class="metric-label">Current Bandwidth</div><div class="metric-value">$([math]::Round($trafficSnapshot.BandwidthMbps,2)) Mbps</div></div>
  <div class="metric-card"><div class="metric-label">Active Connections</div><div class="metric-value">$($trafficSnapshot.TotalConnections)</div></div>
  <div class="metric-card"><div class="metric-label">Top Talkers</div><div class="metric-value">$($trafficSnapshot.TopProcesses.Count)</div></div>
  <div class="metric-card"><div class="metric-label">Sampling Window</div><div class="metric-value">$($trafficSnapshot.DurationSeconds)s</div></div>
</div>
<table style='width:100%; font-size:12px;'>
<tr><th>Process</th><th>PID</th><th>Connections</th><th>Remote IPs (top 5)</th><th>Ports</th><th>Risk</th></tr>
$procRows
</table>
$riskyPortWarning
</div>
"@
    ...
    # Append Speedtest Trend section if available
    if ($speedHistory.Count -gt 0) {
        $rows = ($speedHistory | ForEach-Object {
            $ts = ([datetime]$_.Timestamp).ToString('yyyy-MM-dd HH:mm')
            "<tr><td>$ts</td><td>$([math]::Round($_.DownMbps,1))</td><td>$([math]::Round($_.UpMbps,1))</td><td>$([math]::Round($_.LatencyMs,1))</td><td>$($_.Provider)</td></tr>"
        }) -join ''
        $latest = $speedHistory[-1]
        $html += @"
<div class='section'>
<h2>Bandwidth Speed Tests</h2>
<div class="metrics-grid">
  <div class="metric-card"><div class="metric-label">Latest Download</div><div class="metric-value">$([math]::Round($latest.DownMbps,1)) Mbps</div></div>
  <div class="metric-card"><div class="metric-label">Latest Upload</div><div class="metric-value">$([math]::Round($latest.UpMbps,1)) Mbps</div></div>
  <div class="metric-card"><div class="metric-label">Base Latency</div><div class="metric-value">$([math]::Round($latest.LatencyMs,1)) ms</div></div>
  <div class="metric-card"><div class="metric-label">Tests Recorded</div><div class="metric-value">$($speedHistory.Count)</div></div>
</div>
<table style='width:100%; font-size:12px;'>
<tr><th>Timestamp</th><th>Download (Mbps)</th><th>Upload (Mbps)</th><th>Latency (ms)</th><th>Provider</th></tr>
$rows
</table>
</div>
"@
